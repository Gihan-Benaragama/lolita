...
{
    public int $step = 1; // 1 = Address, 2 = Shipping, 3 = Payment, 4 = Review
    ...
    {
        return $this->shipping_method === 'express' ? 15.00 : 5.00;
    }
    ...
    public function placeOrder(CartService $cartService)
    {
        ...
        foreach ($items as $line) {
            OrderItem::create([
                ...
                'product_name_snapshot' => $line['name'] . ($line['variant_name'] ? ' — ' . $line['variant_name'] : ''),
        ...
        // $order->update(['stripe_payment_intent' => $intent->id, 'status' => 'paid']);
